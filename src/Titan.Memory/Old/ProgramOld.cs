...
unsafe
{
    //var dwSize = 1024ul * 1024ul * 1024ul * 10000ul;
    //var dwSize = ulong.MaxValue-1;
    var dwSize = 64*1024ul;
    for (var i = 0; i < 50; ++i)
    {
        dwSize *= 2;
    }
    var mem = (byte*)Kernel32.VirtualAlloc(null, (nuint)dwSize, AllocationType.MEM_64K_PAGES | AllocationType.MEM_RESERVE, AllocationProtect.PAGE_NOACCESS);


    var commited1 = (byte*)Kernel32.VirtualAlloc(mem, 1024 * 64 * 1024, AllocationType.MEM_COMMIT, AllocationProtect.PAGE_READWRITE);
    var commited2 = (byte*)Kernel32.VirtualAlloc(mem + 1024 * 5 * 64 * 1024, 1024 * 64 * 1024, AllocationType.MEM_COMMIT, AllocationProtect.PAGE_READWRITE);

    var a = Kernel32.VirtualFree(commited2, 1024 * 64 * 1024, AllocationType.MEM_DECOMMIT);

    for (var i = 0; i < 1024 * 64; ++i)
    {
        commited1[i] = unchecked((byte)i);
        commited2[i] = unchecked((byte)i);
    }
